...
object implicits {
  ...
  implicit def fromChannel[T](c: T)(
      implicit toChannel: T => Channel): Option[Channel] = Some(c)
  ...
  def apply(c: Connective): Par =
    new Par(connectives = Vector(c),
            connectiveUsed = ConnectiveLocallyFree.connectiveUsed(c))
  ...
  implicit class ParExtension[T](p: T)(implicit toPar: T => Par) {
    ...
    def prepend(c: Connective, depth: Int): Par =
      p.copy(
        ...
        connectiveUsed = p.connectiveUsed || ConnectiveLocallyFree
          .connectiveUsed(c),
  ...
  implicit val BundleLocallyFree: HasLocallyFree[Bundle] =
    new HasLocallyFree[Bundle] {
      override def connectiveUsed(source: Bundle): Boolean = false
      override def locallyFree(source: Bundle, depth: Int): BitSet =
        source.body.locallyFree
    }

  implicit val SendLocallyFree: HasLocallyFree[Send] =
    new HasLocallyFree[Send] {
      def connectiveUsed(s: Send) = s.connectiveUsed
      def locallyFree(s: Send, depth: Int) = s.locallyFree
    }
  implicit val ExprLocallyFree: HasLocallyFree[Expr] =
    new HasLocallyFree[Expr] {
      def connectiveUsed(e: Expr) =
        e.exprInstance match {
          ...
          case EMatchesBody(EMatches(target, pattern @ _)) =>
            target.connectiveUsed
  ...
  implicit val GPrivateLocallyFree: HasLocallyFree[GPrivate] =
    new HasLocallyFree[GPrivate] {
      def connectiveUsed(g: GPrivate) = false
      def locallyFree(g: GPrivate, depth: Int) = BitSet()
    }

  implicit val ChannelLocallyFree: HasLocallyFree[Channel] =
    new HasLocallyFree[Channel] {
  ...
  implicit val VarLocallyFree: HasLocallyFree[Var] = new HasLocallyFree[Var] {
    def connectiveUsed(v: Var) =
      VarInstanceLocallyFree.connectiveUsed(v.varInstance)
    def locallyFree(v: Var, depth: Int) =
      VarInstanceLocallyFree.locallyFree(v.varInstance, depth)
  }
  ...
  implicit val MatchCaseLocallyFree: HasLocallyFree[MatchCase] =
    new HasLocallyFree[MatchCase] {
      ...
      def locallyFree(mc: MatchCase, depth: Int) =
        mc.source.locallyFree | ParLocallyFree.locallyFree(mc.pattern,
                                                           depth + 1)
    }
