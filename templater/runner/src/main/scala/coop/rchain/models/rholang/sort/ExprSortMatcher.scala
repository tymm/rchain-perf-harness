...
private[sort] object ExprSortMatcher extends Sortable[Expr] {
  private def sortBinaryOperation(p1: Par,
                                  p2: Par): (ScoredTerm[Par], ScoredTerm[Par]) =
  ...
  def sortMatch(e: Expr): ScoredTerm[Expr] = {
    ...
    e.exprInstance match {
      case ENegBody(en) =>
        ...
        constructExpr(ENegBody(ENeg(sortedPar.term)),
                      Node(Score.ENEG, sortedPar.score))
      case EVarBody(ev) =>
        val sortedVar = Sortable.sortMatch(ev.v)
        constructExpr(EVarBody(EVar(sortedVar.term)),
                      Node(Score.EVAR, sortedVar.score))
      case EEvalBody(chan) =>
        val sortedChan = Sortable.sortMatch(chan)
        constructExpr(EEvalBody(sortedChan.term),
                      Node(Score.EEVAL, sortedChan.score))
      case ENotBody(en) =>
        val sortedPar = Sortable.sortMatch(en.p)
        constructExpr(ENotBody(ENot(sortedPar.term)),
      ...
      case EMatchesBody(em) =>
        val (sortedTarget, sortedPattern) =
          sortBinaryOperation(em.target, em.pattern)
        constructExpr(
      ...
      case EPercentPercentBody(ep) =>
        val (sortedPar1, sortedPar2) = sortBinaryOperation(ep.p1, ep.p2)
        constructExpr(EPercentPercentBody(
                        EPercentPercent(sortedPar1.term, sortedPar2.term)),
      ...
      case EMethodBody(em) =>
        ...
        constructExpr(
          EMethodBody(
            em.withArguments(args.map(_.term.get))
              .withTarget(sortedTarget.term.get)),
          Node(
