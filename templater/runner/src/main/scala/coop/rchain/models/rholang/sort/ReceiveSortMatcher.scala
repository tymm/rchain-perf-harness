...
object ReceiveSortMatcher extends Sortable[Receive] {
  ...
  def sortBind(bind: ReceiveBind): ScoredTerm[ReceiveBind] = {
    ...
    val sortedPatterns =
      patterns.toList.map(channel => Sortable.sortMatch(channel))
    ...
    ScoredTerm(
      ReceiveBind(sortedPatterns.map(_.term),
                  sortedChannel.term,
                  bind.remainder,
                  bind.freeCount),
      Node(
        Seq(sortedChannel.score) ++ sortedPatterns.map(_.score) ++ Seq(
          sortedRemainder.score))
    )
  }
  ...
  def sortMatch(r: Receive): ScoredTerm[Receive] = {
    ...
    ScoredTerm(
      Receive(sortedBinds.map(_.term),
              ...
              r.connectiveUsed),
      Node(Score.RECEIVE,
           Seq(Leaf(persistentScore)) ++ sortedBinds.map(_.score) ++ Seq(
             sortedBody.score): _*)
    )
  }
}
