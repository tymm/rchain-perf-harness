  ...
  def sortMatch(g: ExprInstance): ScoredTerm[ExprInstance] =
    g match {
      ...
      case EMapBody(gm) =>
        ...
        val sortedPars =
          gm.ps.sortedMap.map(kv => sortKeyValuePair(kv._1, kv._2)).sorted
        ScoredTerm(
          EMapBody(
            ParMap(sortedPars.map(_.term), gm.connectiveUsed, gm.locallyFree)),
      ...
      case _ => //TODO(mateusz.gorski): rethink it
        throw new IllegalArgumentException(
          "GroundSortMatcher passed unknown Expr instance")
    }
}
