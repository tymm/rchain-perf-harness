...
class Blake2b512Random private (private val digest: Blake2b512Block,
  ...
  override def hashCode(): Int =
    ((digest.hashCode * 31 + pathView
      .position()) * 31 + position) * 31 + lastBlock.hashCode
}
...
object Blake2b512Random {
  ...
  def merge(children: Seq[Blake2b512Random]) = {
    ...
    def internalMerge(children: Vector[Blake2b512Random]): Blake2b512Random = {
      ...
      children.grouped(255).foreach { slice =>
        val result =
          new Blake2b512Random(Blake2b512Block(slice.size.toByte),
                               ByteBuffer.allocate(128))
        squashedBuilder += result
        slice.grouped(2).foreach { pair =>
          pair(0).digest
            .finalizeInternal(pair(0).lastBlock.array(), 0, chainBlock, 0)
          if (pair.size > 1) {
            pair(1).digest
              .finalizeInternal(pair(1).lastBlock.array(), 0, chainBlock, 64)
          } else {
  ...
  implicit val typeMapper = TypeMapper((byteStr: ByteString) => {
    if (byteStr.isEmpty)
      ...
      val digest = Blake2b512Block.typeMapper.toCustom(
        byteStr.substring(16, 16 + digestSize))
    ...
    val totalSize = 16 + digestSize + 2 + rand.pathView
      .position() + remainderSize
    val result = ByteBuffer.allocateDirect(totalSize)
    result
      .order(ByteOrder.LITTLE_ENDIAN)
      .asLongBuffer()
      .put(rand.countView.asReadOnlyBuffer())
    result.position(16)
  ...
  implicit val arbitrary: Arbitrary[Blake2b512Random] = Arbitrary(for {
    ...
    remainder <- Gen.containerOfN[Array, Byte](position,
    ...
    path <- Gen.containerOfN[Array, Byte](pathPosition,
                                          Arbitrary.arbitrary[Byte])
  } yield {
    val result = new Blake2b512Random(digest, ByteBuffer.allocate(128))
