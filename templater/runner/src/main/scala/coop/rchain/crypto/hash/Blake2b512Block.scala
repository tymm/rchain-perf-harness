...
class Blake2b512Block {
  ...
  private[this] def compress(msg: Array[Byte],
    ...
    def g(m1: Long,
      ...
      internalState(posD) =
        rotr64(internalState(posD) ^ internalState(posA), 32);
      internalState(posC) = internalState(posC) + internalState(posD);
      internalState(posB) =
        rotr64(internalState(posB) ^ internalState(posC), 24);
      internalState(posA) = internalState(posA) + internalState(posB) + m2;
      internalState(posD) =
        rotr64(internalState(posD) ^ internalState(posA), 16);
      internalState(posC) = internalState(posC) + internalState(posD);
      internalState(posB) =
        rotr64(internalState(posB) ^ internalState(posC), 63);
    }
    ...
    if (!peek) {
      ...
      for (i <- 0 until CHAIN_VALUE_LENGTH) {
        newChainValue(i) = chainValue(i) ^ internalState(i) ^ internalState(
          i + 8)
      }
    }
  }
...
object Blake2b512Block {
  ...
  val IV: Array[Long] = Array(0x6a09e667f3bcc908L, 0xbb67ae8584caa73bL,
